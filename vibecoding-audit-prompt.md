# Vibecoding Tell Audit & Cleanup — Agent Prompt

A few notes before you use this:

- Paste everything below the divider directly into your coding agent (Claude Code, Cursor, Aider, Windsurf, or similar), pointed at the project's repo root.
- Run it on a clean branch with no uncommitted changes, so every change shows up as a reviewable diff.
- It works best with an existing, passing test suite. If the project doesn't have one, the agent will flag that as a finding rather than silently working without a safety net — worth knowing going in.
...
- Preserve public APIs/interfaces unless the finding is specifically about bad API design — call that out separately, since changing a public interface has broader consequences.
